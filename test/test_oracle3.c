...
static const int smoothing_passes[SMOOTH_LEVELS] = {
    0, 1, 2, 4, 8
};

static const double selectivities[SELECTIVITIES] = {
    0.01, 0.10, 0.50
};
...
static OracleResult
    results[FAMILIES][SMOOTH_LEVELS][SELECTIVITIES][SEEDS];

static double
    neighbor_diff[FAMILIES][SMOOTH_LEVELS][SEEDS];

static double
    eps2[FAMILIES][SMOOTH_LEVELS][SEEDS];
...
static int make_sample(
    ...
    if (!make_noise(
            family, seed, passes, neighbor_mad
        )) {
        return 0;
    }
...
static void metadata_interval(
    ...
    double lo =
        cmin +
        ((double)offsets[2u * block_id] / 255.0)
            * span -
        delta;

    double hi =
        cmin +
        ((double)offsets[2u * block_id + 1u] / 255.0)
            * span +
        delta;
...
static int undecided_count(
    ...
    for (size_t i = 0;
        ...
        int truth =
            (double)reference[i] > threshold;


        double lower = eps == 0.0
            ? v
            : nextafter(v - eps, -INFINITY);

        double upper = eps == 0.0
            ? v
            : nextafter(v + eps, INFINITY);
...
static int evaluate_threshold(
    ...
    for (size_t b = 0; b < BLOCKS; ++b) {
        ...
        if (need_chunk == 0u ||
            ...
            fprintf(
                stderr,
                "FAIL: unresolved at stored precision\n"
            );
            return 0;
        }
    ...
    for (int s = 0; s < SEEDS; ++s) {
        ...
        chunk_saving[s] =
            ...
            : (double)r->static_bytes /
              (double)r->chunk_bytes;

        block_saving[s] =
            r->block_bytes == 0u
            ? 1.0
            : (double)r->static_bytes /
              (double)r->block_bytes;

        need8_fraction[s] =
            r->maybe == 0u
            ? 0.0
            : 100.0 * (double)r->need8_chunk /
              (double)r->maybe;
    ...
    for (int family = 0;
        ...
        for (int seed_index = 0;
            ...
            for (int p = 0;
                ...
                neighbor_diff[family][p][seed_index] =
                    mad;

                eps2[family][p][seed_index] =
                    chunk_eps[0];
