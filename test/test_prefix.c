...
static int encode_reference(
    ...
    actual_rate = zfp_stream_set_rate(zfp, rate, zfp_type_float, 3, 0);
    if (fabs(actual_rate - rate) > 1e-9) {
        fprintf(
            stderr, "Unsupported rate: requested %.1f, actual %.6f\n", rate, actual_rate
        );
        goto cleanup;
    }

    field = zfp_field_3d((void*)input, zfp_type_float, SIDE, SIDE, SIDE);
    if (field == NULL)
        goto cleanup;
...
static int decode_reference(
    ...
    field = zfp_field_3d(output, zfp_type_float, SIDE, SIDE, SIDE);
    if (field == NULL)
        goto cleanup;
...
cleanup:
    ...
    return consumed_bits != 0 && consumed_bits <= prefix_bytes * 8u;
}
...
static void make_sample(float data[VALUES], int sample) {
    for (size_t i = 0; i < VALUES; ++i) {
        ...
        switch (sample) {
        case 0:
            data[i] = 280.0f + 0.31f * x + 0.17f * y + 0.09f * z + 0.03f * sinf(x + y);
            break;
        case 1:
            data[i] =
                5.0f * sinf(0.6f * x + 0.2f * z) - 4.0f * cosf(0.5f * y) + 0.2f * z;
            break;
        case 2:
            data[i] = 293.15f;
            break;
...
int main(void) {
    ...
    const char* samples[] = {"temperature", "wind", "constant", "irregular"};
    ...
    for (int sample = 0; sample < 4; ++sample) {
        ...
        for (size_t level = 0; level < 4; ++level) {
            ...
            if (!encode_reference(input, rate, independent_packed, &independent_size))
                return 1;
            ...
            if (!decode_reference(
                    independent_packed, independent_size, rate, reference_decoded
                ))
                return 1;

            for (size_t b = 0; b < BLOCKS; ++b) {
                const unsigned char* full_block = skipzfp_packed + b * FULL_BLOCK_BYTES;
                const unsigned char* short_block =
                    independent_packed + b * prefix_bytes;

                if (memcmp(full_block, short_block, prefix_bytes) != 0) {
                    fprintf(
                        stderr,
                ...
                if (!decode_only_prefix(
                        full_block, prefix_bytes, rate, prefix_decoded
                    )) {
                    fprintf(
                        stderr,
                        "FAIL %s @ %.0f bits/value block %zu: "
                ...
                extract_reference_block(reference_decoded, b, reference_block);

                if (memcmp(prefix_decoded, reference_block, sizeof(prefix_decoded)) !=
                    0) {
                    fprintf(
                        stderr,
                        "FAIL %s @ %.0f bits/value block %zu: "
