...
enum { POLICY_UNIFORM, POLICY_B16, POLICY_B32, POLICY_B64, POLICY_B128, POLICY_P64 };
...
static const size_t divisors[TOLS] = {100u, 1000u};

static const char* policy_names[POLICIES] = {"U", "B16", "B32", "B64", "B128", "P64"};
...
static StrategyResult results[FAMILIES][PASSES][QUERIES][TOLS][POLICIES][SEEDS];

static size_t baseline_bytes[FAMILIES][PASSES][QUERIES][SEEDS];
...
static int build_query(double target_selectivity, Query* q) {
    memset(q, 0, sizeof(*q));
    ...
    q->threshold = (double)sorted[VALUES - desired - 1u];
    ...
    for (size_t b = 0; b < BLOCKS; ++b) {
        metadata_interval(offsets, b, cmin, cmax, &q->lo[b], &q->hi[b]);

        /* Ground-truth-based safety check ONLY. */
        if ((double)block_min[b] < q->lo[b] || (double)block_max[b] > q->hi[b]) {
    ...
    for (size_t j = 0; j < q->maybe; ++j) {
        ...
        if (!(width > 0.0)) {
            probability = q->lo[b] > q->threshold ? 1.0 : 0.0;
        } else {
            probability = (q->hi[b] - q->threshold) / width;
        ...
        q->estimated_count += BLOCK_VALUES * probability;
    }
...
static int run_uniform(const Query* q, size_t divisor, StrategyResult* out) {
    ...
    for (size_t level = 0; level < LEVELS; ++level) {
        ...
        if (meets_tolerance(lower, unknown, divisor)) {
            out->bytes = q->maybe * LEVEL_BYTES * (level + 1u);

            out->rounds = level + 1u;

            out->reads = q->maybe * (level + 1u);
...
static size_t predict_initial_level(const Query* q, size_t b, size_t divisor) {
    double width = q->hi[b] - q->lo[b];

    double target_unknown = (q->estimated_count / (double)divisor) / (double)q->maybe;
    ...
    for (size_t level = 0; level < 3u; ++level) {
        ...
        double uncertain_lo = q->threshold - eps;

        double uncertain_hi = q->threshold + eps;

        double overlap_lo = fmax(q->lo[b], uncertain_lo);

        double overlap_hi = fmin(q->hi[b], uncertain_hi);

        double overlap = fmax(0.0, overlap_hi - overlap_lo);

        double expected_unknown = BLOCK_VALUES * overlap / width;
...
static int run_online(
    ...
    for (size_t j = 0; j < q->maybe; ++j) {
        ...
        out->bytes += LEVEL_BYTES * (level[j] + 1u);
    ...
    while (!meets_tolerance(lower, unknown, divisor)) {
        ...
        if (n == 0u) {
            fprintf(stderr, "FAIL: no refinable block remains\n");
            return 0;
        }
        ...
        for (size_t k = 0; k < n; ++k) {
            ...
            if (after.in < before.in || after.unknown > before.unknown) {
                ...
                fprintf(stderr, "FAIL: nonmonotone online decision\n");
                return 0;
            }
        ...
        if (!valid_interval(lower, unknown, q->truth)) {
            fprintf(stderr, "FAIL: refinement COUNT interval\n");
            return 0;
        }
    }
    ...
    if (out->bytes > q->static_bytes) {
        fprintf(stderr, "FAIL: fetched more than full MAYBE payload\n");
        return 0;
    }
...
static int
evaluate_policies(const Query* q, size_t divisor, StrategyResult out[POLICIES]) {
    if (!run_uniform(q, divisor, &out[POLICY_UNIFORM]))
        return 0;

    const size_t batches[4] = {16u, 32u, 64u, 128u};

    for (size_t i = 0; i < 4u; ++i) {
        if (!run_online(q, divisor, batches[i], 0, &out[POLICY_B16 + i]))
            return 0;
    }

    if (!run_online(q, divisor, 64u, 1, &out[POLICY_P64]))
        return 0;

    for (size_t p = 0; p < POLICIES; ++p) {
        if (!valid_interval(out[p].lower, out[p].unknown, q->truth)) {
            fprintf(stderr, "FAIL: policy COUNT interval\n");
            return 0;
        }

        if (!meets_tolerance(out[p].lower, out[p].unknown, divisor)) {
...
static double median5(const double values[SEEDS]) {
    double tmp[SEEDS];
...
static void print_summary(int family, int pass_index, int query_index, int tol_index) {
    printf(
        "sel=%2.0f%% tol=%3.1f%% | ",
    ...
    for (int policy = 0; policy < POLICIES; ++policy) {
        ...
        for (int s = 0; s < SEEDS; ++s) {
            const StrategyResult* r =
                &results[family][pass_index][query_index][tol_index][policy][s];

            size_t baseline = baseline_bytes[family][pass_index][query_index][s];

            saving[s] = baseline == 0u || r->bytes == 0u
                            ? 1.0
                            : (double)baseline / (double)r->bytes;
        ...
        for (int s = 0; s < SEEDS; ++s) {
            for (int p = 0; p < PASSES; ++p) {
                ...
                for (int qidx = 0; qidx < QUERIES; ++qidx) {
                    ...
                    if (!build_query(selectivities[qidx], &q)) {

                        fprintf(stderr, "FAIL: build query\n");
                        return 1;
                    }

                    baseline_bytes[family][p][qidx][s] = q.static_bytes;

                    for (int tol = 0; tol < TOLS; ++tol) {

                        StrategyResult one_query[POLICIES];

                        if (!evaluate_policies(&q, divisors[tol], one_query)) {
                            fprintf(
                                stderr,
                                "FAIL: policies family=%d seed=%d "
                                "passes=%d sel=%.2f tol=%zu\n",
                                family,
                                s,
                                passes[p],
                                selectivities[qidx],
                        ...
                        for (int policy = 0; policy < POLICIES; ++policy) {
                            results[family][p][qidx][tol][policy][s] =
                                one_query[policy];
                        }
                    }
                }
