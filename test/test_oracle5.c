...
static const size_t divisors[TOLERANCES] = {
    100u, 1000u
};
...
static QueryResult
    results[FAMILIES][PASSES][QUERIES][SEEDS];
...
static int run_online(
    ...
    while (!meets_tolerance(
        ...
        if (batch_count == 0u) {
            fprintf(
                stderr,
                "FAIL: online has no refinable block\n"
            );
            return 0;
        }
        ...
        for (size_t k = 0;
            ...
            if (after.in < before.in ||
                ...
                fprintf(
                    stderr,
                    "FAIL: online decision not monotone\n"
                );
                return 0;
            }
        ...
        if (!valid_interval(
            ...
            fprintf(
                stderr,
                "FAIL: online refinement interval\n"
            );
            return 0;
        }
    }
...
static int run_hindsight(
    ...
    while (!meets_tolerance(
        ...
        for (size_t j = 0;
            ...
            for (size_t target = old_level + 1u;
                 target < LEVELS;
                 ++target) {


                Decision future = decide_block(
                    b,
                    target,
                    threshold
                );
        ...
        bytes +=
            LEVEL_BYTES *
            (best_target - current_level[best]);
...
static int evaluate_query(
    ...
    for (size_t j = 0;
        ...
        if (!resolved) {
            fprintf(
                stderr,
                "FAIL: exact reference unresolved\n"
            );
            return 0;
        }
    }
    ...
    for (size_t t = 0;
        ...
        if (out->uniform[t].bytes >
            ...
            out->online[t].bytes >
                out->static_bytes ||
            out->hindsight[t].bytes >
                out->static_bytes) {

            fprintf(
                stderr,
                "FAIL: strategy fetched too many bytes\n"
            );
            return 0;
        }
    }
...
static void print_summary(
    ...
    for (int s = 0;
        ...
        const StrategyResult *u =
            &r->uniform[tolerance_index];

        const StrategyResult *o =
            &r->online[tolerance_index];

        const StrategyResult *h =
            &r->hindsight[tolerance_index];
    ...
    min_median_max(
        exact, &lo, &exact_med, &hi
    );

    min_median_max(
        uniform, &lo, &uniform_med, &hi
    );

    min_median_max(
        online, &lo, &online_med, &hi
    );

    min_median_max(
        hindsight, &lo, &hindsight_med, &hi
    );
