...
static const int smooth_passes[PASSES] = {0, 1, 2, 4, 8};

static const double selectivities[QUERIES] = {0.01, 0.10, 0.50};

static const size_t tolerance_divisor[TOLERANCES] = {100u, 1000u};
...
static int make_noise(int family, uint32_t seed, int passes) {
    uint32_t state = seed;
...
static int make_sample(int family, uint32_t seed, int passes) {
...
static int prepare_sample(int family, uint32_t seed, int passes) {
    ...
    for (size_t b = 0; b < BLOCKS; ++b) {
        ...
        for (size_t level = 0; level < LEVELS; ++level) {
            ...
            if (!decode_prefix(payload, bytes, rates[level], decoded[level][b])) {
                fprintf(stderr, "FAIL: prefix decode block=%zu level=%zu\n", b, level);
                return 0;
            }
...
static int build_decisions(size_t block_id, double threshold) {
    for (size_t level = 0; level < LEVELS; ++level) {
    ...
    for (size_t i = 0; i < BLOCK_VALUES; ++i) {
        ...
        double truth_value = (double)decoded[3][block_id][i];

        for (size_t level = 0; level < LEVELS; ++level) {
            double v = (double)decoded[level][block_id][i];
            ...
            if (lower > upper || truth_value < lower || truth_value > upper) {
                fprintf(
                    stderr,
    ...
    for (size_t level = 1; level < LEVELS; ++level) {
        if (decisions[level][block_id].in < decisions[level - 1u][block_id].in ||
            decisions[level][block_id].unknown >
...
static int valid_answer(size_t lower, size_t unknown, size_t truth) {
    return lower <= truth && unknown <= VALUES - lower && truth - lower <= unknown;
}

static int evaluate_uniform(size_t divisor, size_t truth, AnytimeResult* out) {
    for (size_t level = 0; level < LEVELS; ++level) {
        ...
        if (meets_tolerance(lower, unknown, divisor)) {
            out->bytes = maybe_count * 16u * (level + 1u);
...
static int evaluate_adaptive(size_t divisor, size_t truth, AnytimeResult* out) {
    ...
    while (!meets_tolerance(lower, unknown, divisor)) {
        ...
        for (size_t j = 0; j < maybe_count; ++j) {
            ...
            for (size_t target = current + 1u; target < LEVELS; ++target) {

                size_t before = decisions[current][b].unknown;

                size_t after = decisions[target][b].unknown;
                ...
                if (best_j == maybe_count || gain * best_steps > best_gain * steps ||
        ...
        size_t gained_in = decisions[best_target][b].in - decisions[old][b].in;

        size_t resolved = decisions[old][b].unknown - decisions[best_target][b].unknown;
...
static int evaluate_query(double target_selectivity, QueryResult* out) {
    ...
    for (size_t b = 0; b < BLOCKS; ++b) {
        ...
        metadata_interval(offsets, b, cmin, cmax, &lo, &hi);

        if ((double)block_min[b] < lo || (double)block_max[b] > hi) {
    ...
    for (size_t t = 0; t < TOLERANCES; ++t) {
        ...
        if (!evaluate_uniform(divisor, truth, &out->uniform[t]))
            return 0;

        if (!evaluate_adaptive(divisor, truth, &out->adaptive[t]))
            return 0;

        if (out->uniform[t].bytes > out->static_bytes ||
            out->adaptive[t].bytes > out->static_bytes) {

            fprintf(stderr, "FAIL: bytes exceed full scan\n");
            return 0;
        }

        if (!valid_answer(out->adaptive[t].lower, out->adaptive[t].unknown, truth)) {
            return 0;
        }
    }
...
static void print_summary(int family, int pass_index, int query_index) {
    double exact[SEEDS];
    ...
    for (int s = 0; s < SEEDS; ++s) {
        ...
        double baseline = (double)r->static_bytes;

        exact[s] = r->exact_bytes == 0u ? 1.0 : baseline / (double)r->exact_bytes;

        uniform_1[s] =
            r->uniform[0].bytes == 0u ? 1.0 : baseline / (double)r->uniform[0].bytes;

        adaptive_1[s] =
            r->adaptive[0].bytes == 0u ? 1.0 : baseline / (double)r->adaptive[0].bytes;

        uniform_01[s] =
            r->uniform[1].bytes == 0u ? 1.0 : baseline / (double)r->uniform[1].bytes;

        adaptive_01[s] =
            r->adaptive[1].bytes == 0u ? 1.0 : baseline / (double)r->adaptive[1].bytes;

        interval_1[s] = (double)r->adaptive[0].unknown;
    }
    ...
    min_median_max(adaptive_1, &lo, &a1_med, &hi);

    min_median_max(uniform_01, &lo, &u01_med, &hi);

    min_median_max(adaptive_01, &lo, &a01_med, &hi);

    min_median_max(interval_1, &lo, &interval_med, &hi);
    ...
    for (int family = 0; family < FAMILIES; ++family) {
        for (int s = 0; s < SEEDS; ++s) {
            for (int p = 0; p < PASSES; ++p) {
                ...
                if (!prepare_sample(family, seed_values[s], smooth_passes[p])) {
                    fprintf(
                        stderr,
